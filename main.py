...
def split_listings_by_quality(listings: List[dict]) -> tuple[List[dict], List[dict]]:
    ...
    """
    nq_listings = [
        listing["pricePerUnit"] for listing in listings if listing.get("hq") == False
    ]
    hq_listings = [
        listing["pricePerUnit"] for listing in listings if listing.get("hq") == True
    ]
...
def calculate_market_stats(listings: List[dict]) -> Dict:
    ...
    d = {"nq": nq_prices, "hq": hq_prices}
    # Calculate stats
    try:
        for k, v in d.items():
            stats[k] = {"medianPrice": median(v), "minPrice": min(v)}
    except:
        pass
    return stats
...
def get_ingredients(item: int) -> Optional[list]:
    ...
    with duckdb.connect(config.DB_NAME) as db:
        try:
            ...
            df = db.sql(
                f"SELECT * FROM main.recipe_price WHERE result_id = {item}"
            ).pl()
...
def fetch_universalis(item_id: int, region: str = "Japan") -> Optional[dict]:
    ...
    try:
        ...
        for ingredient, details in item_data.items():
            ...
            all_listings = [
                listing
                for listing in item_market.get("listings", [])
        ...
        def print_formatted_price_data(item: dict) -> None:
            ...
            print(f"Shop price: {item['shop_unit']:,.0f}") if item[
                "shop_unit"
            ] else None
            print(f"Shop price x Number: {item['shop_total']:,.0f}") if item[
                "shop_total"
            ] and item["amount"] > 1 else None
            print(f"Marketboard NQ price: {item['nq_unit']:,.0f}") if item[
                "nq_unit"
            ] else None
            print(f"Marketboard NQ price x Number: {item['nq_total']:,.0f}") if item[
                "nq_total"
            ] and item["amount"] > 1 else None
            print(
                f"Marketboard NQ velocity: {item['nq_velocity']:,.0f} items/day"
            ) if item["nq_velocity"] else None
            print(f"Marketboard HQ price: {item['hq_unit']:,.0f}") if item[
                "hq_unit"
            ] else None
            print(f"Marketboard HQ price x Number: {item['hq_total']:,.0f}") if item[
                "hq_total"
            ] and item["amount"] > 1 else None
            print(
                f"Marketboard HQ velocity: {item['hq_velocity']:,.0f} items/day"
            ) if item["hq_velocity"] else None
        ...
        for k, v in item_data.items():
            if k.startswith("ingredient"):
                ...
                nq_craft_cost += min(
                    x
                    for x in [item["shop_total"], item["nq_total"], item["hq_total"]]
                    if x is not None
                )
                ...
                if item["hq_total"] is not None:
                    hq_craft_cost += min(
                        x
                        for x in [item["shop_total"], item["hq_total"]]
                        if x is not None and item["hq_total"]
                    )
                else:
                    hq_craft_cost += min(
                        x
        ...
        print(
            f"Craft HQ from buying NQ items cost: {nq_craft_cost:,.0f}"
        ) if nq_craft_cost else None
        print(
            f"Craft HQ from buying HQ items cost: {hq_craft_cost:,.0f}"
        ) if hq_craft_cost else None
        print("")
        print(
            f"Craft HQ from buying NQ items P/L: {buy_hq_total:,.0f} - {nq_craft_cost:,.0f} = {nq_craft_pl:,.0f} ({nq_craft_pl_perc:,.2%})"
        )
